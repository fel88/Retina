...
namespace Retina
{
    public class FaceNet
    {
        ...
        public Tuple<RecFaceInfo, double?> Recognize(Mat mat)
        {
            ...
            return new Tuple<RecFaceInfo, double?>(best, maxdist);
        }
        public float[] Inference(Mat mat)
        {
            if (session == null)
            {
                session = new InferenceSession("nets\\facenet.onnx");
            }
            ...
            {
                mat = mat.Resize(new OpenCvSharp.Size(inputMeta[name].Dimensions[2], inputMeta[name].Dimensions[1]));
